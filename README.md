...
### Доступ сотрудников в локальной сети

- На компьютере, где запущен paperCall: `http://localhost`
- С другого компьютера в той же LAN: `http://<LAN-IP-компьютера-с-сервером>`
- `localhost` на телефоне или другом компьютере указывает на само устройство, а не на компьютер с paperCall.
- Для LAN-доступа используйте IP компьютера-сервера, например `http://192.168.x.x`.
- Не публикуйте paperCall в Интернет без отдельной настройки TLS/VPN/firewall.
- Внешним входом остаётся Nginx на порту `80`; backend `8080`, MySQL, Redis, Kafka и AMI наружу не открывайте.
